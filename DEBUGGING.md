...
**Choosing validation in code:** The variables above only set the default. `Instance::with_validation(goldy::Validation { .. })` picks the checks for one instance, whatever the environment says, and `Runtime::validation()` reports what a runtime runs. Use it in tests that must run with a check off (for example CUDA graph-capture tests, since validated CUDA backends never capture) or on (for example `host_access`). Metal's `MTL_SHADER_VALIDATION` is the exception: it is process-wide, so only the first Metal backend's choice takes effect. Some API validation cannot be scoped to one instance: the D3D12 debug layer follows the build profile and `GOLDY_DX12_DEBUG`, `VK_INSTANCE_LAYERS` forces the Khronos layer on every Vulkan instance, and `CUDA_LAUNCH_BLOCKING` affects every CUDA backend. When one of these contradicts an instance's `gpu_api`, Goldy logs a warning on the `goldy::validation` target, once per process.
