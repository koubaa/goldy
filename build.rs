...
fn main() {
    ...
    let manifest = match load_manifest(&manifest_path) {
        ...
        Err(e) => {
            ...
            generate_empty_embedded_module("unknown");
    ...
    generate_embedded_module(&manifest.version, &slang_bin_dir, platform_info, &platform_dir);
    emit_goldy_cache_version(Some(manifest.version.as_str()));
}
...
fn generate_empty_embedded_module(platform_dir: &str) {
    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    let embedded_path = out_dir.join("slang_embedded.rs");

    let content = format!(
        r#"// Auto-generated: Slang binaries not available for this platform.

/// Slang version (empty - not available)
pub const SLANG_VERSION: &str = "";

/// Cargo target platform this crate was built for (not the build-host).
pub const SLANG_EMBED_PLATFORM: &str = "{platform_dir}";
...
fn generate_embedded_module(version: &str, vendored_dir: &Path, platform_info: &PlatformInfo, platform_dir: &str) {
    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    let embedded_path = out_dir.join("slang_embedded.rs");
    ...
    content.push_str(&format!(
        "/// Cargo target platform this crate was built for (not the build-host).\n\
         pub const SLANG_EMBED_PLATFORM: &str = \"{}\";\n\n",
        platform_dir
    ));
...
/// Slang zip/platform folder for the **cargo target**, not the machine running build.rs.
/// Cross-compiling linux-aarch64 on an x86_64 runner must embed linux-aarch64 libraries.
fn get_platform_dir() -> String {
    if let Ok(explicit) = env::var("GOLDY_SLANG_PLATFORM") {
