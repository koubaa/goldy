...
|------|------------|---------|
| [`Runtime`](./runtime-owned-memory.md) | Cross-submission identity (deeds) | `acquire_texture` / `acquire_buffer` / `acquire_record` |
| Context transient pool | One-submission tenancy (leases) | `Context::lease_texture` / `lease_buffer` / `lease_render_target`, or `acquire_transient_texture` / `acquire_transient_buffer` |
...
scattered suballocation — see `Runtime::acquire_record`. Do not construct
