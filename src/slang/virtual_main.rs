...
//! [shader("compute")]
//! [numthreads(64, 1, 1)]
//! void cs_main(uniform uint _p0, uniform uint _p1, uint3 _sv0 : SV_DispatchThreadID) {
//!     TimeUniforms cfg = goldy_broadcast<TimeUniforms>(_p0);
//!     Scattered<uint> data = goldy_scattered<uint>(_p1);
...
impl WrapperBuilder {
    ...
    fn process_param(&mut self, param: &Param, push_idx: &mut u32, sv_idx: &mut u32, pt_idx: &mut u32) {
        match &param.kind {
            ParamKind::Resource => {
                let gn = format!("_p{}", *push_idx);
                *push_idx += 1;
                self.push_sig(&format!("uniform uint {}", gn));
                let init_expr = resource_init_expr(&param.ty, &gn);
    ...
    for item in &entry.params {
        match item {
            ParamItem::Single(param) => {
                wb.process_param(param, &mut push_idx, &mut sv_idx, &mut pt_idx);
            }
...
"#;
        let result = transform_virtual_main(src);
        // Should have two uniform params: _p0 (Scattered) and _p1 (uint base).
        assert!(result.contains("uniform uint _p0"), "Missing _p0");
        assert!(result.contains("uniform uint _p1"), "Missing _p1 for scalar");
        // base is a scalar — passed directly, no wrapper construction.
        assert!(result.contains("_goldy_user_cs_main(data, id, _p1)"), "Wrong call args");
    }
...
"#;
        ...
        // Unconditional param before the conditional.
        assert!(result.contains("uniform uint _p0"), "Missing _p0 (config)");
...
"#;
        ...
        // Resource resolved via goldy_scattered.
        assert!(result.contains("Scattered<uint> data = goldy_scattered<uint>(_p1)"), "Missing resource init");
    }
...
"#;
        ...
        assert!(result.contains("SV_DispatchThreadID"), "Missing SV_DispatchThreadID");
        // Wrapper calls goldy_* free functions to resolve slots into handles.
        assert!(result.contains("goldy_interpolated<float4>(_p0)"), "Missing src_tex goldy_interpolated init");
        assert!(result.contains("goldy_filter(_p1)"), "Missing samp goldy_filter init");
        assert!(result.contains("goldy_scattered<float4>(_p2)"), "Missing dst goldy_scattered init");
    }
}
