...
//! `__goldy_padN` fields.
//!
//! Three-component vectors are emitted as three scalars plus a Slang `property`
//! (`v.pos` still type-checks as `float3`). A native `float3` field is 16 bytes
//! in Metal structured buffers, which would desync the packer from reflection.
...
impl GpuFieldType {
    ...
    const fn vec3_scalar_slang_name(self) -> Option<&'static str> {
        match self {
            Self::F32x3 => Some("float"),
            Self::U32x3 => Some("uint"),
            Self::I32x3 => Some("int"),
            _ => None,
        }
    }
...
impl GpuType<'_> {
    ...
    pub(crate) fn generate(&self) -> Result<GeneratedGpuType> {
        ...
        for (field, packed_field) in self.fields.iter().zip(packed.fields.iter()) {
            ...
            if let Some(scalar) = field.ty.vec3_scalar_slang_name() {
                let bx = format!("__goldy_c3_{}_x", field.name);
                let by = format!("__goldy_c3_{}_y", field.name);
                let bz = format!("__goldy_c3_{}_z", field.name);
                source.push_str(&format!("    {scalar} {bx};\n"));
                source.push_str(&format!("    {scalar} {by};\n"));
                source.push_str(&format!("    {scalar} {bz};\n"));
                reflected_fields.push((bx.clone(), packed_field.storage_offset, 4));
                reflected_fields.push((by.clone(), packed_field.storage_offset + 4, 4));
                reflected_fields.push((bz.clone(), packed_field.storage_offset + 8, 4));
                let vec_ty = field.ty.slang_name();
                properties.push_str(&format!(
                    "    property {name} : {vec_ty} {{\n\
                         get {{ return {vec_ty}({bx}, {by}, {bz}); }}\n\
                         set(newValue) {{ {bx} = newValue.x; {by} = newValue.y; {bz} = newValue.z; }}\n\
                     }}\n",
                    name = field.name,
                ));
            } else {
...
mod tests {
    ...
    fn generated_struct_inserts_internal_and_tail_padding() {
        ...
        assert!(generated.source.contains("__goldy_c3_position_x;"));
        assert!(generated.source.contains("property position : float3"));
        assert!(generated.source.contains("uint __goldy_pad0;"));
        assert!(generated.source.contains("float2 uv;"));
        assert_eq!(
            generated.check.rust_fields,
            vec![
                ("__goldy_c3_position_x".into(), 0, 4),
                ("__goldy_c3_position_y".into(), 4, 4),
                ("__goldy_c3_position_z".into(), 8, 4),
    ...
    #[test]
    fn scalar_triple_needs_no_author_padding() {
        ...
        assert!(Uniforms::GPU_TYPE.storage_matches_host().unwrap());
        let generated = Uniforms::GPU_TYPE.generate().unwrap();
        assert!(
            !generated.source.contains("__goldy_pad"),
            "scalar triple must not inject pad fields:\n{}",
            generated.source
        );
        assert!(
            !generated.source.contains("_padding"),
            "author padding must not appear:\n{}",
            generated.source
        );
        let host = Uniforms {
            width: 8,
    ...
    fn tight_float3_then_float2_matches_structured_buffer_stride() {
        ...
        let result = compiler
            ...
            .expect("generated struct must compile and reflect for Metal");
        assert_eq!(result.reflection.binding_element_strides[0], Some(32));
    ...
    fn doom_static_vertex_spirv_bufro_stride_matches_array_stride() {
        ...
        compiler
            ...
            .expect("StaticVertex BufRO Metal");
