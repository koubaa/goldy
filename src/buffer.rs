...
/// Bump arena over one scattered backing buffer (internal; used by [`crate::TransientAllocator`]).
...
impl BufferPool {
    pub(crate) fn with_alignment_capacity_hint_and_flags(
        device: &Device,
        total_size: u64,
        expected_max: u64,
        alignment: u64,
    ...
    pub(crate) fn alloc_bytes(&mut self, size: u64, element_stride: Option<u32>) -> Result<BufferView> {
        bump_scattered_subregion(&self.backing, &mut self.offset, self.alignment, size, element_stride)
    }
    ...
    pub(crate) fn hint_unused_above(&mut self, offset: u64) {
        self.backing.hint_unused_above(offset);
    }

    #[cfg(test)]
    pub(crate) fn padded_size(allocs: &[(usize, usize)]) -> u64 {
        scattered_suballoc_padded_size(allocs)
    }

    #[cfg(test)]
    pub(crate) fn new(device: &Device, total_size: u64) -> Result<Self> {
        Self::with_alignment(device, total_size, 256)
    }

    #[cfg(test)]
    fn with_alignment(device: &Device, total_size: u64, alignment: u64) -> Result<Self> {
        assert!(alignment.is_power_of_two(), "alignment must be a power of two");
    ...
    #[cfg(test)]
    pub(crate) fn alloc<T: StructuredBufferElement>(&mut self, count: u64) -> Result<BufferView> {
        let stride = std::mem::size_of::<T>() as u64;
        let size = count * stride;
    ...
    #[cfg(test)]
    pub(crate) fn alloc_with_data<T: StructuredBufferElement>(&mut self, data: &[T]) -> Result<BufferView> {
    ...
    #[cfg(test)]
    pub(crate) fn read_to_cpu(&self, device: &Device, output: &mut [u8]) -> Result<()> {
