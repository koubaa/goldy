...
use goldy::{Context, DepositTarget, DepositTransaction, GoldyError, MemoryExchange, Parcel, Scheme, Submission};
...
pub fn bind_upload_deposit(
    ...
    MemoryExchange::new(ctx).bind_deposit(upload, DepositTarget::buffer(parcel, capacity))
}
...
pub fn write_to_parcel(ctx: &Context, parcel: &Parcel, data: &[u8]) -> Result<Submission, GoldyError> {
    ...
    let deposit =
        MemoryExchange::new(ctx).bind_deposit(&mut upload, DepositTarget::buffer(parcel, data.len() as u64))?;
