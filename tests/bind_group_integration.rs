...
fn test_bind_group_layout_creation() {
    ...
    assert!(layout.is_ok(), "Failed to create bind group layout: {:?}", layout.err());
}
...
fn test_bind_group_layout_multiple_entries() {
    ...
    assert!(layout.is_ok(), "Failed to create multi-entry bind group layout: {:?}", layout.err());
}
...
fn test_bind_group_with_uniform_buffer() {
    ...
    assert!(bind_group.is_ok(), "Failed to create bind group: {:?}", bind_group.err());
}
...
fn test_pipeline_with_bind_group() {
    ...
    let shader = ShaderModule::from_slang(&device, UNIFORM_SHADER)
    ...
    assert!(pipeline.is_ok(), "Failed to create pipeline with bind group layout: {:?}", pipeline.err());
}
...
fn test_render_with_uniform_buffer() {
    ...
    assert!(has_colored, "Expected colored pixels from uniform buffer rendering");
}
...
fn test_multiple_bind_groups() {
    ...
    let bind_group1 = BindGroup::new(&device, &layout, &[
        BufferBinding::new(0, &uniform1),
    ])
    .expect("Failed to create bind group 1");

    let bind_group2 = BindGroup::new(&device, &layout, &[
        BufferBinding::new(0, &uniform2),
    ])
...
fn test_bind_group_with_buffer_range() {
    ...
    assert!(bind_group.is_ok(), "Failed to create bind group with buffer range: {:?}", bind_group.err());
}
...
fn test_storage_buffer_binding() {
    ...
    let bind_group = BindGroup::new(&device, &layout, &[
        BufferBinding::new(0, &storage_buffer),
    ]);

    assert!(bind_group.is_ok(), "Failed to create bind group with storage buffer: {:?}", bind_group.err());
}
