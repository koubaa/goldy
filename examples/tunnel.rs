...
//!
//! Demonstrates fragment shader with polar coordinate transform using Surface API.
//!
...
fn create_quad(time: f32) -> [TunnelVertex; 6] {
    [
        TunnelVertex { position: [-1.0, -1.0], uv: [0.0, 1.0], time },
        TunnelVertex { position: [1.0, -1.0], uv: [1.0, 1.0], time },
        TunnelVertex { position: [1.0, 1.0], uv: [1.0, 0.0], time },
        TunnelVertex { position: [-1.0, -1.0], uv: [0.0, 1.0], time },
        TunnelVertex { position: [1.0, 1.0], uv: [1.0, 0.0], time },
        TunnelVertex { position: [-1.0, 1.0], uv: [0.0, 0.0], time },
    ]
}
        ...
        if self.window.is_none() {
            let window = Arc::new(event_loop.create_window(
                Window::default_attributes()
                    .with_title("RAG - Tunnel Effect (Surface API)")
...
fn main() -> anyhow::Result<()> {
    ...
    println!("RAG Tunnel Example - Press Escape to exit");
